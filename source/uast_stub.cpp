...
namespace uast {
    ...
    Node build(const std::string& filePath,
        ...
        if (!javaVmPath.empty()   && javaVmPath.back()   != '/') javaVmPath   += '/';
        if (!pythonVmPath.empty() && pythonVmPath.back() != '/') pythonVmPath += '/';
        ...
        boost::property_map<graph_t,std::string DotEdge::*>::type nameMap =
            get(&DotEdge::name, graph);
        ...
        for (boost::tie(vIt,end) = boost::vertices(graph); vIt != end; vIt++) {
            ...
            for (boost::tie(eIt,edge_end) = boost::out_edges(*vIt, graph);
                 eIt != edge_end; eIt++) {
        ...
        auto rootP = std::find_if(degrees.begin(), degrees.end(), [](const auto& p) {
          return p.second == 0;
        });
